	...
	set_card_actions(actions) {
			...
			this.card_actions = $(`
				<div class="card-actions dropdown pull-right">
					<a data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">...</a>
					<ul class="dropdown-menu" style="max-height: 300px; overflow-y: auto;">
						${actions
							.map(action => {
								if (frappe.user.has_role('Administrator') || (action.label == "Refresh")) {
									return `<li class="dropdown-item"><a data-action="${action.action}">${action.label}</a></li>`;
								} else {
									// Handle case where user does not have 'System Manager' role
									return '';
								}
							})
							.join("")}
