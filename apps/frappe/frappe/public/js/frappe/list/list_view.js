...
frappe.views.ListView = class ListView extends frappe.views.BaseList {
	...
	get_header_html() {
		...
		if (!frappe.user.has_role("System Manager")){
			subject_html =`
				<input class="level-item list-check-all hide" type="checkbox"
					title="${__("Select All")}" disabled>
				<span class="level-item mx-3" data-sort-by="${subject_field.fieldname}"
					title="${__("Click to sort by {0}", [subject_field.label])}">
					${__(subject_field.label)}
				</span> `
		}
...
class ElementFactory {
   ...
   if (!frappe.user.has_role("System Manager")  || undefined) {
			checkbox.classList.add("list-row-checkbox","hide");
		} else {
