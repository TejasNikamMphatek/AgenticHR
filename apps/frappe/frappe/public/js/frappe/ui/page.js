// Copyright (c) 2015, Pipal ERP and Contributors
 ...
 */
hide_view_list = require('./hide_view_list.json')
...
frappe.ui.Page = class Page {
	...
	add_dropdown_item({
		...
		let login_user_role = frappe.user.has_role("System Manager")
		if(login_user_role == undefined){
			login_user_role = false; // False if System Manager role is not present
		} // True or Undefined
		if (show_parent && login_user_role) {
		...
		$link = $li.find("a").on("click", (e) => {
			...
			if (standard) {
				...
				if (!this.divider.length) {
					let admin_yes = frappe.user.has_role(['Administrator'])
					this.divider = $('<li class="dropdown-divider user-action hide"></li>').prependTo(
						parent
					);
				}
	...
	getDoctypeHideViewData(current_doctype) 
	{
		...
		if (doctype_hide_list) {
			// console.log(" Hide View Available",doctype_hide_list);
			return doctype_hide_list;
		} else {
			// console.log("No data found for the specified doctype");
			return {
				"System Manager" : [],
				"Other User" : []
			};
