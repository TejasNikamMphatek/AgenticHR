...
class AutoEmail(Document):
	...
	def validate_report_format(self):
		"""Check if user has selected a correct report format"""
		valid_report_formats = ["HTML", "XLSX", "CSV", "PDF"]
		if self.format not in valid_report_formats:
			frappe.throw(
				_("{0} is not a valid report format. Report format should be one of the following: {1}").format(
	...
	def validate_mandatory_fields(self):
		...
		if throw_list:
			frappe.throw(
				...
				msg=_("Following Report Filters have missing values:") + "<br><br><ul><li>" + " <li>".join(throw_list) + "</ul>",
			)

	def get_report_content(self, requested_format=None):
		"""Returns file in the specified or document's format"""
		report = frappe.get_doc("Report", self.report)
		original_format = self.format
		self.format = requested_format if requested_format in ["HTML", "XLSX", "CSV", "PDF"] else self.format
		...
		try:
			if self.format == "HTML":
				columns = update_field_types(columns)  # Convert Link fields to Data, no hyperlinks
				...
				frappe.log_error(f"Invalid format {self.format} for report {self.name}", "Auto Email Download")
	...
	def get_file_name(self):
		...
		if self.format == "PDF":
			ext = "pdf"
		return "{}.{}".format(self.report.replace(" ", "-").replace("/", "-"), ext)
...
def download(name):
	...
	# Get requested format from query parameters, fallback to document format
	requested_format = frappe.form_dict.get("format", auto_email.format)
	if requested_format not in ["HTML", "XLSX", "CSV", "PDF"]:
		frappe.log_error(f"Invalid requested format {requested_format} for Auto Email {name}", "Auto Email Download")
		requested_format = auto_email.format  # Fallback to document format

	data = auto_email.get_report_content(requested_format)
	if not data:
		frappe.msgprint(_("No Data"))
		return

	# Set appropriate MIME type and headers for download
	mime_types = {
		"HTML": "text/html",
	...
	frappe.local.response.mimetype = mime_types.get(requested_format, "application/octet-stream")

	# Log for debugging
	frappe.log_error(
		f"Downloading Auto Email {name} in format {requested_format} with filename {frappe.local.response.filename}",
		"Auto Email Download Debug"
	)
