...
class AutoEmail(Document):
    ...
    def validate_report_format(self):
        """Check if user has selected correct report format"""
        valid_report_formats = ["HTML", "XLSX", "CSV", "PDF"]
        if self.format not in valid_report_formats:
            frappe.throw(
                _("{0} is not a valid report format. Report format should be one of: {1}").format(
    ...
    def get_report_content(self):
        """Returns file for the report in given format"""
        ...
        if self.format == "HTML":
            ...
            columns, data = make_links(columns, data)
            columns = update_field_types(columns)
            html_content = self.get_html_table(columns, data)
            try:
            ...
            report_data = frappe._dict(columns=columns, result=data)
            ...
            report_data = frappe._dict(columns=columns, result=data)
            xlsx_data, _ = build_xlsx_data(report_data, [], 1, ignore_visible_idx=True)
...
@frappe.whitelist()
def download(name, format=None):
    """Download report locally"""
    auto_email = frappe.get_doc("Auto Email", name)
    auto_email.check_permission()

    # If format parameter is passed, override doc format
    if format and format in ["HTML", "XLSX", "CSV", "PDF"]:
        original_format = auto_email.format
        auto_email.format = format
        data = auto_email.get_report_content()
        auto_email.format = original_format
    else:
        data = auto_email.get_report_content()
    ...
    current_format = format if format else auto_email.format
    file_extension = current_format.lower()
    filename = "{}.{}".format(auto_email.report.replace(" ", "-").replace("/", "-"), file_extension)
    ...
    if current_format in mime_types:
        frappe.local.response.headers = {
            "Content-Type": mime_types[current_format],
...
def send_daily():
    ...
    for report in enabled_reports:
        ...
        if auto_email.frequency == "Weekdays" and current_day in ("Saturday", "Sunday"):
            continue
        if auto_email.frequency == "Weekly" and auto_email.day_of_week != current_day:
            continue
        try:
            auto_email.send()
        except Exception:
...
def make_links(columns, data):
    for row in data:
        ...
        for col in columns:
            ...
            elif col.fieldtype == "Dynamic Link" and col.options and row.get(col.options):
