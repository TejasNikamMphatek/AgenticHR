	...
	frappe.pages['project-manager-dash'].on_page_show = function(wrapper) {
		...
		frappe.project_manager_dash = {
			...
			run: function () {
				...
				frappe.call({
					method: "hrms.hr.page.project_manager_dash.project_manager_dash.getProManData",
					args: {
						start: me.start,
					},
					callback: function (response){
						...
						{
							$('#page-project-manager-dash').addClass('hidden')
						}
					},
				});
				me.updateTime();
			...
			{	
				...
				$(frappe.render_template("project_manager_dash",data)).appendTo(me.page.main);
				this.startClock()
			...
			viewSwipe: function () {
				...
				todays_swipe = me.manager_data[2];
				var swipe_data = me.manager_data[2]['todaysSwipe'];
