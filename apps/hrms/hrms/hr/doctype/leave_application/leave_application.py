...
class LeaveApplication(Document, PWANotificationsMixin):
	...
	def before_save(self):
		...
		if self.leave_approver:
			leave_approver_data = frappe.db.get_all(
				"Employee",
				filters={
					"status" : "active",
					"user_id" : self.leave_approver
				},
				fields = ["first_name","last_name"],
				order_by="user_id DESC",
        		limit_page_length=1 
			)
			if leave_approver_data[0]['first_name']:
				self.leave_approver_name = f"{leave_approver_data[0]['first_name']}  {leave_approver_data[0]['last_name']}"

		
	def validate_privious_application(self):
		if self.from_date and self.to_date:
			from_date_obj = getdate(self.from_date)
			check_holi_date = from_date_obj - datetime.timedelta(days=1)
			is_holiday = self.validate_holiday_off(check_holi_date)
			...
			if is_holiday:
				leave_applications = frappe.db.get_all(
					...
					fields=["employee", "leave_type", "from_date", "to_date", "total_leave_days"],
				...
				if leave_applications:
					...
					privious_app_end_date = getdate(leave_applications[0]['to_date'])
					if privious_app_end_date < from_date_obj:
						currect_application_start_date = from_date_obj

						# Get all dates between privious_app_end_date and currect_application_start_date
						date_list = []
						temp_date = privious_app_end_date + datetime.timedelta(days=1)
						
						while temp_date < currect_application_start_date:
							date_list.append(temp_date)
							temp_date += datetime.timedelta(days=1)
						# print(date_list)

						# Check if all dates are holidays
						all_holidays = all(self.validate_holiday_off(date) for date in date_list)

						
						if all_holidays:
							next_working_day = privious_app_end_date + datetime.timedelta(days=1)
							self.from_date = next_working_day
							frappe.msgprint(msg="Sandwich Leave applied", alert=True, indicator="orange")
	...
	def validate(self):
		...
		# Add functionality of sandwich leave
		sandwich_status = frappe.db.get_single_value("HR Settings", "sandwich_leave")
		if sandwich_status:
			self.validate_privious_application()
	...
	def validate_dates(self):
		...
		if self.half_day and self.half_day_date:
			is_holiday = self.validate_holiday_off(self.half_day_date)
			if is_holiday:
				self.half_day_date = None
				frappe.throw(_("Half Day Date cannot be selected on a holiday date."))
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			# print("Attendance Name === ",attendance_name)
