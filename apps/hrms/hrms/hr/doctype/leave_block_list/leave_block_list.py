    ...
    def validate(self):
        ...
        for d in self.get("leave_block_list_dates"):
            if d.block_date in dates:
                frappe.msgprint(
                    _("Date is repeated") + ":" + d.block_date,
                    raise_exception=1
                )
            dates.append(d.block_date)

        # ---------------------------------------------------
        # 🔹 Duplicate allowed users (GitHub new validation)
        # ---------------------------------------------------
        allow_users = []
        for row in self.get("leave_block_list_allowed"):   # Correct child table
            if row.allow_user in allow_users:
                frappe.throw(
                    _("Duplicate Allow User found: {0}. Each user can only be added once.")
                    .format(row.allow_user),
                    title=_("Duplicate User")
                )
            allow_users.append(row.allow_user)
