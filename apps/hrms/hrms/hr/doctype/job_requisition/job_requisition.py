# Fixed Python Code (job_requisition.py)
...
@frappe.whitelist()
def get_pending_referral_count(designation=None):
    """
    Get count of pending employee referrals for a specific designation
    
    Args:
        designation (str): The designation to count referrals for
    
    Returns:
        int: Number of pending referrals, 0 if designation is empty or on error
    """
    try:
        # Return 0 if designation is not provided or empty
        ...
        count = frappe.db.count(
            "Employee Referral",
            {
                "for_designation": designation.strip(),
                "status": "Pending"
            }
        )
        ...
        frappe.log_error(
            message=f"Error in get_pending_referral_count for designation '{designation}': {str(e)}",
            title="Job Requisition Referral Count Error"
        )
        return 0