...
from frappe.utils import format_date, get_link_to_form, getdate,get_days_in_month
...
class LeaveEncashment(Document):
	...
	def set_actual_encashable_days(self):
		...
		if encashment_settings.max_encashable_leaves:
			...
			if encashment_settings.max_encashable_leaves < self.encashment_days:
				frappe.msgprint(
	...
	def set_encashment_amount(self):
		...
		emp_salary_structure_assignment = frappe.get_all(
		...
		monthday = get_days_in_month(self.encashment_date)
		
		if emp_salary_structure_assignment:
			emp_base_salary = emp_salary_structure_assignment[0]['base']
			monthly_basic_salary = (emp_base_salary / 12) * 0.30
			per_day_encashment = monthly_basic_salary / monthday
			# print(per_day_encashment)
