...
def get_data(filters: Filters) -> list:
	leave_types = get_leave_types(filters.company)
...
def get_leave_types(company) -> list[str]:
    LeaveType = frappe.qb.DocType("Leave Type")
    query = frappe.qb.from_(LeaveType).select(LeaveType.name).orderby(LeaveType.name).where(LeaveType.company == company)
