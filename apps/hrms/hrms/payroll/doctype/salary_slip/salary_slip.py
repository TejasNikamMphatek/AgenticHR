# Copyright (c) 2015, Pipal ERP Pvt. Ltd. and Contributors
...
class SalarySlip(TransactionBase):
	def __init__(self, *args, **kwargs):
		...
		month_year = frappe.utils.get_datetime(self.start_date).strftime('%B %Y')
		self.series = f"Payslip_{self.employee}_{month_year}_"
	...
	def before_save(self):
		release_sal = frappe.db.get_all(
			"Hold Salary Employee",
			filters={
				"employee": self.employee,
				"salary_release": 1
			},
			fields=["salary_release", "salary_release_date","last_working_day"],
			limit=1,
		)
		if release_sal:
			if getdate(self.posting_date) >= getdate(release_sal[0].salary_release_date):
				frappe.msgprint(
					_(f"Salary slip is creating "),
					alert=True,
					indicator="green",
				)
			elif getdate(self.posting_date) <= getdate(release_sal[0].salary_release_date):
				frappe.throw(
                    f"Employee <b>{self.employee_name}</b> is not eligible this month because 45 days not completed after LWD."
                )
		else:
			emp_status = frappe.db.get_all(
				"Employee",
				filters={
					"name": self.employee,
				},
				fields=["status", "employee_number"],
				limit=1,
			)

			if emp_status:
				if emp_status[0].status in ["Suspended", "Inactive"]:
					frappe.throw(
						f"Employee {self.employee_name} is not eligible for creation due to the Employee Status {emp_status[0].status}."
					)

			resignation_emp = frappe.db.get_all(
				"Employee Separation",
				filters={
					"employee": self.employee,
					"docstatus": 1,
				},
				fields=["approved_lwd", "stop_salary_process_date"],
				limit=1,
			)

			if resignation_emp:
				stop_salary_process_date = getdate(resignation_emp[0].get("stop_salary_process_date"))
				start_date = getdate(self.start_date)
				end_date = getdate(self.end_date)

				if stop_salary_process_date and start_date <= stop_salary_process_date <= end_date:
					frappe.throw(
						f"Employee {self.employee_name} is not eligible for creation due to the Stop Salary Process."
					)
				elif stop_salary_process_date <= start_date :
					frappe.throw(
						f"Employee {self.employee_name} is not eligible this month because 45 days not completed after Resign."
					)

		# ----Payroll Lock-unlock validation----start------ 
		if self.payroll_period:
			payroll_period = self.payroll_period['name']

			lock_unclock_payroll = frappe.get_all(
				"Lock Unlock Payroll",
				filters=[["select_payroll_period", "=", payroll_period]],
				fields=["*"] 
			)
		
			if lock_unclock_payroll:
				payroll_start_date = lock_unclock_payroll[0]['payroll_start_date']
				payroll_end_date = lock_unclock_payroll[0]['payroll_end_date']
				months = [
					
					('apr', lock_unclock_payroll[0].apr, lock_unclock_payroll[0].apr_lock_date),
					('may', lock_unclock_payroll[0].may, lock_unclock_payroll[0].may_lock_date),
					('jun', lock_unclock_payroll[0].jun, lock_unclock_payroll[0].jun_lock_date),
					('jul', lock_unclock_payroll[0].jul, lock_unclock_payroll[0].jul_lock_date),
					('aug', lock_unclock_payroll[0].aug, lock_unclock_payroll[0].aug_lock_date),
					('sep', lock_unclock_payroll[0].sep, lock_unclock_payroll[0].sep_lock_date),
					('oct', lock_unclock_payroll[0].oct, lock_unclock_payroll[0].oct_lock_date),
					('nov', lock_unclock_payroll[0].nov, lock_unclock_payroll[0].nov_lock_date),
					('dec', lock_unclock_payroll[0].dec, lock_unclock_payroll[0].dec_lock_date),
					('jan', lock_unclock_payroll[0].jan, lock_unclock_payroll[0].jan_lock_date),
					('feb', lock_unclock_payroll[0].feb, lock_unclock_payroll[0].feb_lock_date),
					('mar', lock_unclock_payroll[0].mar, lock_unclock_payroll[0].mar_lock_date)
				]
				start_date = getdate(self.start_date)
				end_date = getdate(self.end_date)
				for month_name, month_lock, month_lock_date in months:
					if(month_lock_date):
						month_lock_date  = getdate(month_lock_date)
						if (start_date <= month_lock_date <= end_date) and month_lock:
								frappe.throw(_(f"Locked the Payroll of <b>{month_name.upper()}</b>. Can not do change"))
						elif month_lock_date and (not month_lock):
							frappe.msgprint(
								_(f"Payroll still unlock of  <b>{month_name.upper()}</b> Month."),
								alert=True,
								indicator="orange",
							)
	...
	def validate(self):
		...
		if not (len(self.get("earnings")) or len(self.get("deductions"))):
			...
			# print("This is leave_without_pay ",self.leave_without_pay)
	...
	def set_net_total_in_words(self):
		...
		# print(doc_currency,company_currency)
	...
	def get_working_days_details(self, lwp=None, for_preview=0):
		...
		if payroll_settings.payroll_based_on == "Attendance":
			actual_lwp, ppl, absent = self.calculate_lwp_ppl_and_absent_days_based_on_attendance(
				holidays, daily_wages_fraction_for_half_day, consider_marked_attendance_on_holidays
			)
			self.absent_days = absent
			self.partially_paid_leave = ppl

		else:
			actual_lwp , ppl = self.calculate_lwp_or_ppl_based_on_leave_application(
				holidays, working_days_list, daily_wages_fraction_for_half_day
			)
			self.partially_paid_leave = ppl
			# lwp = actual_lwp

		# print("actual_lwp .........--- ",actual_lwp) # coorectly working for "Attendance"
		# print("lwp ...........--- ",lwp)
		# actual_lwp = lwp
		if not lwp:
			lwp = actual_lwp
		...
		self.total_working_days = working_days
		# print("total_working_days = imp " ,working_days)
		# print("lwp = imp " ,lwp)
	...
	def get_unmarked_days(
		...
		# print("self.total_working_days",self.total_working_days)
		# print("self._get_days_outside_period",self._get_days_outside_period(include_holidays_in_total_working_days, holidays))
		# print("self._get_marked_attendance_days(holidays)",self._get_marked_attendance_days(holidays))
		unmarked_days = (
	...
	def calculate_lwp_or_ppl_based_on_leave_application(
		...
		for d in working_days_list:
			...
			if cint(leave.half_day) and (leave.half_day_date == d or leave.from_date == leave.to_date):
				...
				if not cint(leave.is_ppl) and cint(leave.half_day):
					equivalent_lwp_count = (1 - daily_wages_fraction_for_half_day) if is_half_day_leave else 0.500
					lwp += equivalent_lwp_count
					# print ("equivalent_lwp_count-- half day condition----", equivalent_lwp_count)
				elif cint(leave.is_ppl) and cint(leave.half_day):
					equivalent_ppl_count = (1 - daily_wages_fraction_for_half_day) if is_half_day_leave else 0.500
					ppl += equivalent_ppl_count


			if cint(leave.is_ppl) and not cint(leave.half_day):
				equivalent_ppl_count = 1
				equivalent_ppl_count *= (
					fraction_of_daily_salary_per_leave if fraction_of_daily_salary_per_leave else 1
				)
				ppl += equivalent_ppl_count
				# print ("equivalent_ppl_count--is_ppl condition----", equivalent_ppl_count)

			elif not cint(leave.is_ppl) and not cint(leave.half_day):
				equivalent_lwp_count = 1
				equivalent_lwp_count *= (
					fraction_of_daily_salary_per_leave if fraction_of_daily_salary_per_leave else 1
				)
				lwp += equivalent_lwp_count
				# print ("equivalent_lwp_count--NOT is_ppl condition----", equivalent_lwp_count)
				
			# print ("649 lwp---++++++++++++++++++++++++++++++++++++++++++-", lwp, "  ppl-----",ppl)
	...
	def calculate_lwp_ppl_and_absent_days_based_on_attendance(
		...
		for d in attendance_details:
			...
			if d.status == "Half Day":
				...
				if d.leave_type in leave_type_map.keys() and not leave_type_map[d.leave_type]["is_ppl"]:
					equivalent_lwp = 1 - daily_wages_fraction_for_half_day
					lwp += equivalent_lwp

				elif d.leave_type in leave_type_map.keys() and leave_type_map[d.leave_type]["is_ppl"]:
					equivalent_ppl = 1 - daily_wages_fraction_for_half_day
					ppl += equivalent_ppl

			elif d.status == "On Leave" and d.leave_type and d.leave_type in leave_type_map.keys():
				equivalent_lwp = 1
				equivalent_ppl = 1
				if not leave_type_map[d.leave_type]["is_ppl"]:
					equivalent_lwp *= (
						fraction_of_daily_salary_per_leave if fraction_of_daily_salary_per_leave else 1
					)
					lwp += equivalent_lwp
				elif leave_type_map[d.leave_type]["is_ppl"]:
					equivalent_ppl *= (
						fraction_of_daily_salary_per_leave if fraction_of_daily_salary_per_leave else 1
					)
					ppl += equivalent_ppl
				...
				absent += 1
		# print("lwp==",lwp,"   absent==",absent,"   ppl==",ppl)
	...
	def calculate_net_pay(self):
		...
		# print("self.gross_pay****************%%%%%%%*****************",self.gross_pay)
	...
	def compute_annual_deductions_before_tax_calculation(self):
		...
		for d in self.get("deductions"):
			if d.exempted_from_income_tax:
				# print("exempted_from_income_tax = d.amount = ",d.amount)
		...
		# print("prev_period_exempted_amount : ", prev_period_exempted_amount)
		# print("current_period_exempted_amount : ", current_period_exempted_amount)
		# print("future_period_exempted_amount : ", future_period_exempted_amount)
		return (
	...
	def get_data_for_eval(self):
		...
		for key in ("earnings", "deductions"):
			for d in self.get(key):
				# print("for d in self.get(key):",d.default_amount)
	...
	def calculate_variable_based_on_taxable_salary(self, tax_component):
		if not self.payroll_period:
			...
			return
		# print(self.calculate_variable_tax(tax_component),"self.calculate_variable_tax(tax_component)=0=0=0=0=0=0")
	...
	def calculate_variable_tax(self, tax_component):
		...
		# print("total_taxable_earnings_without_full_tax_addl_components= ",self.total_taxable_earnings_without_full_tax_addl_components)
		...
		self.total_structured_tax_amount = calculate_tax_by_tax_slab(
			...
			self.whitelisted_globals,
			eval_locals,
			self.tax_slab.is_new_regime
		)
		# print("self.total_structured_tax_amount == ",self.total_structured_tax_amount)

		current_month_income_tax_amt = 0
		for deduction in eval_locals.deductions:
			if deduction.salary_component == "Income Tax" and deduction.abbr == "IT":
				# print(deduction.amount,"------")
				current_month_income_tax_amt = deduction.amount
		# self.current_month_income_tax_amount = current_month_income_tax_amt

		# print("default_data default_data default_data == ",default_data)
		# print("total_structured_tax_amount",self.total_structured_tax_amount)
		# print("previous_total_paid_taxes",self.previous_total_paid_taxes)
		# print("self.remaining_sub_periods",self.remaining_sub_periods)
		...
		# if(isinstance(self.remaining_sub_periods,int)):
		# 	self.current_structured_tax_amount = (
		# 		self.total_structured_tax_amount - self.previous_total_paid_taxes
		# 	) / self.remaining_sub_periods
		# elif isinstance(self.remaining_sub_periods,float):
		# 	fractional_part, integer_part = math.modf(self.remaining_sub_periods)
		# 	if (fractional_part):
		# 		self.current_structured_tax_amount = (self.total_structured_tax_amount - self.previous_total_paid_taxes ) * fractional_part
				
		# for deduction in self.deductions:
		# 	print(self.get_amount_based_on_payment_days(deduction))

		# print("self.current_structured_tax_amount",self.current_structured_tax_amount)
			
		# Total taxable earnings with additional earnings with full tax
		self.full_tax_on_additional_earnings = 0.0
		if self.current_additional_earnings_with_full_tax:
			# print("self.tax_slab.is_new_regime=",self.tax_slab.is_new_regime)
			self.total_tax_amount = calculate_tax_by_tax_slab(
				self.total_taxable_earnings, self.tax_slab, self.whitelisted_globals, eval_locals,self.tax_slab.is_new_regime
			)
		...
		# print("previous_total_paid_taxes= = ******",self.previous_total_paid_taxes,"total_structured_tax_amount= = ******",self.total_structured_tax_amount,"current_structured_tax_amount= = ******",self.current_structured_tax_amount,"full_tax_on_additional_earnings= = ******",self.full_tax_on_additional_earnings,"current_tax_amount= = ******",current_tax_amount)
	...
	def get_taxable_earnings_for_prev_period(self, start_date, end_date, allow_tax_exemption=False):
		# print("get_taxable_earnings_for_prev_period====")
	...
	def get_tax_paid_in_period(self, start_date, end_date, tax_component):
		...
		# print("tax_deducted_till_date",tax_deducted_till_date)
	...
	def get_total_exemption_amount(self):
		...
		if not self.tax_slab.allow_tax_exemption and self.tax_slab.is_new_regime:
			if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
				...
				# print("Exemption Proof == ",exemption_proof)
				if exemption_proof:
		...
		elif self.tax_slab.allow_tax_exemption :
			if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
				exemption_proof = frappe.db.get_value(
					"Employee Tax Exemption Proof Submission",
					{"employee": self.employee, "payroll_period": self.payroll_period.name, "docstatus": 1},
	...
	def process_salary_structure(self, for_preview=0):
		...
		# print("**************",self.get_working_days_details(for_preview=for_preview))
		self.calculate_net_pay()
		...
		if self.salary_slip_based_on_timesheet == 1:
			...
			if hasattr(self, "earnings"):
				for earning in self.earnings:
					...
					# print(self.gross_pay,"Test Gross Amount")
...
def calculate_tax_by_tax_slab(annual_taxable_earning, tax_slab, eval_globals=None, eval_locals=None, is_new_regime=None):
	...
	if annual_taxable_earning <= 700000 and is_new_regime :
		...
		for slab in tax_slab.slabs:
			...
			if not slab.to_amount and annual_taxable_earning >= slab.from_amount:
				...
				# print("tax_amount last round==",tax_amount,"annual_taxable_earning=",annual_taxable_earning)
				continue

			if annual_taxable_earning >= slab.from_amount and annual_taxable_earning < slab.to_amount:
				tax_amount += (annual_taxable_earning - slab.from_amount + 1) * slab.percent_deduction * 0.01
				# print("tax_amount 2= ",tax_amount,"annual_taxable_earning - slab.from_amount + 1=",annual_taxable_earning - slab.from_amount + 1)

			elif annual_taxable_earning >= slab.from_amount and annual_taxable_earning >= slab.to_amount:
				tax_amount += (slab.to_amount - slab.from_amount + 1) * slab.percent_deduction * 0.01
				# print("tax_amount 1= ",tax_amount,"annual_taxable_earning=",annual_taxable_earning)
		tax_amount_data.append(tax_amount)
	...
	# print ("tax_amount_data", tax_amount_data)
	# It will rounded clearly like cleartax calculator
	tax_amount = round(min(tax_amount_data) / 10)* 10
	...
	for d in tax_slab.other_taxes_and_charges:
		...
		if flt(d.max_taxable_income) and flt(d.max_taxable_income) < annual_taxable_earning:
			continue
		other_tax_charges = tax_amount * flt(d.percent) / 100
		# print("other_tax_charges on income tax (edu.cess)= ",other_tax_charges)
