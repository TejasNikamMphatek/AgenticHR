# Copyright (c) 2017, Pipal ERP  and contributors
...
class PayrollEntry(Document):
	...
	def fill_employee_details(self):
		...
		employees = get_employee_list(self,filters=filters, as_dict=True, ignore_match_conditions=True,)
		self.set("employees", [])
...
def get_employee_list(self,
    ...
    # Retrieve hold and release salary data
    hold_release_emp = hold_release_salary_emp()
    ...
    held_employee_ids = {emp['employee'] for emp in hold_release_emp['sal_hold_emp']}

    filtered_employees = {
        emp_id: emp_data
        for emp_id, emp_data in employees_to_check.items()
        if emp_id not in held_employee_ids
	}

    for emp in hold_release_emp['sal_release_emp']:
        emp_id = emp['employee']
        salary_release_date = emp.get('salary_release_date')
        salary_hold_date = emp.get('salary_hold_date')
        last_working_day = emp.get('last_working_day')
       
        if salary_release_date and salary_hold_date:
            posting_date = getdate(self.posting_date)
            start_date = getdate(self.start_date)
            end_date = getdate(self.end_date)
            salary_release_date = getdate(salary_release_date)
            salary_hold_date = getdate(salary_hold_date)
            last_working_day = getdate(last_working_day)

            if salary_hold_date == last_working_day:
                if (salary_release_date <= posting_date) and (start_date <= salary_hold_date <= end_date):
                    if emp_id in employees_to_check:
                        filtered_employees[emp_id] = employees_to_check[emp_id]
                else:
                    filtered_employees.pop(emp_id, None)
            else:
                if (
					salary_release_date <= posting_date
					and ((start_date <= salary_hold_date <= end_date) or (start_date <= last_working_day <= end_date))
				):
                	...
                	filtered_employees.pop(emp_id, None)
    return remove_payrolled_employees(filtered_employees, filters.start_date, filters.end_date)

def hold_release_salary_emp():
	sal_hold_emp = frappe.db.get_all(
		"Hold Salary Employee",
		filters={"salary_hold": 1},
		fields=["employee","salary_hold","salary_hold_date", "last_working_day"],
	)

	sal_release_emp = frappe.db.get_all(
		"Hold Salary Employee",
		filters={"salary_release": 1},
		fields=["employee","salary_release","salary_release_date","salary_hold_date", "last_working_day"],
	)
	return {"sal_hold_emp":sal_hold_emp, "sal_release_emp":sal_release_emp}
