...
class Employee(NestedSet):
	...
	def before_save(self):
		...
		ttl_prev_exp = 0
		for prev_exp in self.external_work_history:
			try:
				experience = float(prev_exp.total_experience)
			except ValueError:
				experience = 0  
			ttl_prev_exp += experience
		self.total_work_experience = ttl_prev_exp

		# Internal Work History Validatio for FromDate and ToDate
		# for internal_date in self.internal_work_history:
		# 	try:
		# 		if internal_date.from_date and internal_date.to_date:
		# 			if internal_date.from_date > internal_date.to_date:
		# 				frappe.throw(_("Enter Valid Dates of Internal Work History "))
		# 		else:
		# 			frappe.throw(_("Both <b>From Date</b> and <b>To Date</b> must be provided."))
		# 	except ValueError:
		# 		frappe.throw(_("Enter Valid Dates!"))

		# Set Auto Calculate Confirmation_Date
		if isinstance(self.date_of_joining, str):
			joining_date = datetime.strptime(self.date_of_joining, '%Y-%m-%d').date()
		elif isinstance(self.date_of_joining, date):
